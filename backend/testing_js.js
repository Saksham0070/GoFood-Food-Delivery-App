let a = ["Apple","Orange","Banana","Apple"]

a.filter(item=>item!="Apple").map(data=>{
...
foodCategory.map(data=>{
    console.log("FoodCategory = ",data)

    foodItems.length>0?
        foodItems.filter(item=>item.CategoryName===data.CategoryName)
        .map(filterItems=>{
            return (console.log(filterItems))
        })
    :console.log("No Item of this category")
})
